(function () {
  ...
  function productCard(p) {
    const image = productImages(p)[0] || 'veyrath-tee.jpg';
  ...
  function openProduct(p) {
    if (!p) return;
    const modal = $('#productModal'); const gallery = productImages(p); if (!gallery.length) gallery.push('veyrath-tee.jpg'); const galleryMarkup = gallery.map((image, index) => `<img src="${esc(image)}" alt="${esc(p.name)} gallery image ${index + 1}" ${index ? 'loading="lazy"' : ''}>`).join('');
    $('#productModalBody').innerHTML = `<div class="modal-grid"><div class="modal-gallery"><div class="modal-images" data-modal-track>${galleryMarkup}</div>${gallery.length > 1 ? `<div class="modal-gallery-controls"><button type="button" data-gallery-prev aria-label="Previous product image">←</button><span><b data-gallery-current>1</b> / ${gallery.length}</span><button type="button" data-gallery-next aria-label="Next product image">→</button></div>` : ''}</div><div class="modal-copy"><p class="eyebrow">${esc(p.category || 'VEYRATH')}</p><h2>${esc(p.name)}</h2><div class="price price-large"><strong>${money(p.sale_price || p.price)}</strong>${Number(p.compare_at_price || 0) > Number(p.sale_price || p.price || 0) ? `<s>${money(p.compare_at_price)}</s>` : ''}</div><p>${esc(p.description || 'A VEYRATH piece shaped for silent movement.')}</p><dl><div><dt>Sizes</dt><dd>${esc(split(p.sizes).join(' / ') || 'One size')}</dd></div><div><dt>Colours</dt><dd>${esc(split(p.colours).join(' / ') || 'As shown')}</dd></div><div><dt>Made</dt><dd>${esc(p.fulfilment_status || 'After order')}</dd></div></dl><button class="btn btn-gold" type="button" data-buy-now="${esc(p.id)}">Buy securely</button><small>Payment protected by Razorpay.</small></div></div>`;
    const track = $('[data-modal-track]', modal); const counter = $('[data-gallery-current]', modal); let activeImage = 0; const moveGallery = (next) => { activeImage = (next + gallery.length) % gallery.length; track.scrollTo({ left: activeImage * track.clientWidth, behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' }); if (counter) counter.textContent = String(activeImage + 1); }; $('[data-gallery-prev]', modal)?.addEventListener('click', () => moveGallery(activeImage - 1)); $('[data-gallery-next]', modal)?.addEventListener('click', () => moveGallery(activeImage + 1)); track?.addEventListener('scroll', () => { const next = Math.round(track.scrollLeft / Math.max(1, track.clientWidth)); if (next !== activeImage && next >= 0 && next < gallery.length) { activeImage = next; if (counter) counter.textContent = String(activeImage + 1); } }, { passive: true });
    modal.showModal();
  }
