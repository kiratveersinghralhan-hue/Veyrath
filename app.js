(function () {
  ...
  function home() {
    ...
    const offers = data.offers?.length ? data.offers : [{ kicker: 'BORN AFTER DARK', text: data.marquee_text || '' }]; const offerSet = offers.map((o) => `<article><small>${esc(o.kicker || 'VEYRATH')}</small><strong>${esc(o.text || '')}</strong><span>✦</span></article>`).join(''); $('[data-offer-rail]').innerHTML = `<div class="offer-set">${offerSet}</div><div class="offer-set" aria-hidden="true">${offerSet}</div>`;
  ...
  function setupCarousel(banners) {
    ...
    const track = $('.campaign-track', host); const slides = $$('.campaign-slide', host); const dots = $$('[data-carousel-dot]', host); let current = 0; let timer = null; let startX = 0; const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  function reveal() { const candidates = $$('.reveal,.section-head,.value-card,.support-card,.prose>*,.footer-grid>*,.product-card'); candidates.forEach((el, i) => { el.classList.add('reveal'); if (!el.dataset.reveal) el.dataset.reveal = i % 3 === 0 ? 'left' : i % 3 === 1 ? 'up' : 'right'; }); if (matchMedia('(prefers-reduced-motion: reduce)').matches || !('IntersectionObserver' in window)) return candidates.forEach((el) => el.classList.add('is-visible')); const io = new IntersectionObserver((entries) => entries.forEach((e) => e.target.classList.toggle('is-visible', e.isIntersecting)), { threshold: .1, rootMargin: '-4% 0px -4%' }); candidates.forEach((el) => io.observe(el)); }
  function intro() { if (document.body.dataset.page !== 'home' || matchMedia('(prefers-reduced-motion: reduce)').matches) return; document.body.classList.add('intro-lock'); document.body.insertAdjacentHTML('afterbegin', `<div class="luxury-intro" role="presentation"><button class="intro-skip" type="button">Skip intro</button><div class="intro-lines" aria-hidden="true"><i></i><i></i></div><div class="luxury-mark" aria-hidden="true"><div class="luxury-orbit"><i></i><i></i><i></i></div><div class="intro-word"><span>V</span><span>E</span><span>Y</span><span>R</span><span>A</span><span>T</span><span>H</span></div><small>Born After Dark</small></div><div class="intro-curtain intro-curtain-left"></div><div class="intro-curtain intro-curtain-right"></div></div>`); const el = $('.luxury-intro'); const finish = () => { if (!el || el.classList.contains('is-leaving')) return; el.classList.add('is-leaving'); document.body.classList.remove('intro-lock'); setTimeout(() => el.remove(), 1100); }; requestAnimationFrame(() => el.classList.add('is-ready')); $('.intro-skip', el).addEventListener('click', finish); setTimeout(finish, 2850); }
