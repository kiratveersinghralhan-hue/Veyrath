(function () {
  ...
  function fillProduct(product) {
    ...
    $('#checkoutSecurityNote').hidden = false;
    ...
    try {
      ...
      const razorpay = new window.Razorpay({
        ...
        handler: async (response) => {
          ...
          form.reset();
          $('#checkoutTitle').textContent = 'Your order is confirmed.';
          const fulfilmentAccepted = Boolean(verified?.auto_fulfilment?.printrove_order_id || verified?.auto_fulfilment?.already_created);
          $('#checkoutSuccessNumber').textContent = verified.order_number;
          $('#checkoutSuccessCopy').textContent = fulfilmentAccepted
            ? 'Payment verified and production request accepted. No further action is required.'
            : 'Payment verified. Your order is secured and queued for fulfilment review.';
          $('#checkoutTrackLink').href = `track.html?order=${encodeURIComponent(verified.order_number)}`;
          $('#checkoutSuccess').hidden = false;
          $('#checkoutSecurityNote').hidden = true;
          message('A secure payment receipt has been issued. Tracking follows after dispatch.', 'success');
